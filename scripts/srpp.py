...
def main():
    ...
    eval_callback = EvalCallback(env, best_model_save_path="./src/SRPP/scripts/logs/sac_Franka_best_model_circle",
                                 log_path="./src/SRPP/scripts/logs/sac_Franka_eval", eval_freq=eval_freqency,
    ...
    model = SAC.load("./src/SRPP/scripts/logs/sac_Franka_modelCircle_4", env=env)
    # model.load_replay_buffer("./src/SRPP/scripts/logs/sac_replay_buffer")
    model.learn(total_timesteps=update_k_everysteps, log_interval=10, tb_log_name="Circle_Tracking",
                callback=callback,
                reset_num_timesteps=False)
    model.save("./src/SRPP/scripts/logs/sac_Franka_model" + "Circle_5")
    ...
    for i in range(1, k_update_times + 1):
        ...
        # Train the network
        model = SAC.load("./src/SRPP/scripts/logs/sac_Franka_model" + f"Circle_{4+i}", env=env)
        model.learn(total_timesteps=update_k_everysteps, log_interval=10, tb_log_name="Circle_Tracking",
                    callback=callback,
                    reset_num_timesteps=False)

        # Save the network at the end
        model.save("./src/SRPP/scripts/logs/sac_Franka_model" + f"Circle_{4+i + 1}")
        # model.save_replay_buffer("./src/SRPP/scripts/logs/sac_replay_buffer")
