...
def main():
    ...
    while num_curves_done < num_curves:
        ...
        try:
            start = np.array(
                list(map(float, input(f"请输入第 {num_curves_done + 1} 条曲线的起点（格式：x y z）: ").split())))
            control = np.array(
                list(map(float, input(f"请输入第 {num_curves_done + 1} 条曲线的控制点（格式：x y z）: ").split())))
            end = np.array(
                list(map(float, input(f"请输入第 {num_curves_done + 1} 条曲线的终点（格式：x y z）: ").split())))
