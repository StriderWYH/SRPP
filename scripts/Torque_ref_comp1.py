...
class TorqueCircleTrajectory(gymnasium.Env):
    ...
    def reset(self, seed=None, options=None, startstate=None):
        ...
        if self.time is not None and self.time > 0:
            ...
            self.error[4] = self.max_dist_error
    ...
    def _compute_reward(self, w_dist=0.5, w_joint=0.3, w_ori=0, w_vel=0.2):
        ...
        d_error = np.linalg.norm((self.state_xyz - self.ref_xyz))
        x_d = d_error/100
        ...
        self.rms_error += d_error ** 2
        self.max_dist_error = self.max_dist_error if self.max_dist_error >= d_error else d_error
