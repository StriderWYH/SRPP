...
class PretrainedEnv(gym.Env):
    ...
    def __init__(self, thr=50, weight=0.00069, vel=4.0, accel=4.0):
        ...
        self.observation_space = spaces.Box(low=-10.0, high=10.0, shape=(23,), dtype=np.float64)
        ...
        time.sleep(2.0)
        self.target_orientation = self.limb.endpoint_pose()['orientation']  # this is an array of type quaternion
    ...
    def getRandomPointInCircle(self):
        samplePoint = [] # millimeter
        sampleAngle = []
        while True:
            for i in range(2):  # generate two point
                while True:
                    theta = random.random() * 2 * math.pi
                    phi = random.random() * math.pi
                    r = 100 + 700 * random.random()  # the range is within [100,800]
                    x = r * np.sin(phi) * np.cos(theta)
                    y = r * np.sin(phi) * np.sin(theta)
                    z = r * np.cos(phi)
                    temp_check = self.limb.inverse_kinematics(np.array([x / 1000, y / 1000, z / 1000]),
                                                              self.target_orientation)
                    if temp_check[0] is True:
                        samplePoint.append(np.array([x, y, z]))
                        sampleAngle.append(temp_check[1])
                        break
            if np.linalg.norm(samplePoint[0] - samplePoint[1]) > 100:
                break
            else:                # if two point is too close
                samplePoint = []
                sampleAngle = []

        return (samplePoint, sampleAngle)
    ...
    def reset(self, startstate=None):
        ...
        sample = self.getRandomPointInCircle()
        ...
        temp = [self.state_xyz[0] / 100, self.state_xyz[1] / 100, self.state_xyz[2] / 100, self.target_xyz[0] / 100,
                self.target_xyz[1] / 100, self.target_xyz[2] / 100, self.linear_vel[0], self.linear_vel[1],
                self.linear_vel[2]]
