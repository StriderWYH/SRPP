...
class PretrainedEnv3(gym.Env):
    ...
    def __init__(self, thr=20, weight=0.00069, vel=4.0, accel=4.0,length =60):
        ...
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=(49,), dtype=np.float64)
        # 定义动作空间
    ...
    def reset(self, startstate=None):
        ...
        state = np.hstack((self.joint_state/PI, self.joint_state_error / PI, alter_torque_state,alter_pre_torque_state,
                           self.pre_joint_state/PI,self.target_joint_state/PI))
